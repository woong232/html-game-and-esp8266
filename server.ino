...
void setup() {
  ...
  server.on("/ledRED",ledRED); //Set setver all paths are not found so we can handle as per URI
  server.on("/ledGREEN",ledGREEN);
  server.on("/ledBLUE",ledBLUE);
  server.on("/ledCYAN",ledCYAN);
  server.on("/ledYELLOW",ledYELLOW);
  server.on("/ledMAGENTA",ledMAGENTA);
